...
function renderAgencyBaseline(){
  ...
  const linked=all.filter(x=>x.item.centerRequirementId).length,open=all.filter(x=>x.item.status!=='done').length;
  ...
  $('#servicesGrid').innerHTML=`<section class="center-baseline-overview"><article class="glass chart-card"><header><div><p class="eyebrow">مؤشر خط أساس الوكالة</p><h2>روستر المتطلبات وسجلات الأعمال</h2></div><span class="source-chip">قاعدة محافظ الوكالة · ${ar(all.length)} سجل</span></header><div class="agency-kpi-grid"><div><b>${ar(all.length)}</b><span>إجمالي السجل</span></div><div><b>${ar(linked)}</b><span>مرتبطة بالمركز</span></div><div><b>${ar(all.length-linked)}</b><span>بانتظار المطابقة</span></div><div><b>${ar(open)}</b><span>جارية أو قادمة</span></div><div><b>${ar(new Set(all.map(x=>x.p.id)).size)}</b><span>محافظ ذات سجلات</span></div></div></article></section><section class="baseline-source-head"><div><span>خط الأساس التشغيلي للوكالة</span><b>المتطلبات وسجلات الأعمال المسجلة في المحافظ</b><small>الأولوية موروثة من المحفظة أو المسار الفرعي؛ تاريخ القيد يظهر عندما لا يتوفر تاريخ طلب صريح.</small></div><div class="baseline-filters"><label>المحفظة<select id="agencyPortfolioFilter"><option value="all">جميع المحافظ</option>${portfolios.map(x=>`<option ${state.agencyPortfolio===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label>الحالة<select id="agencyStatusFilter"><option value="all">جميع الحالات</option>${statuses.map(x=>`<option ${state.agencyStatus===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label>الأولوية<select id="agencyPriorityFilter"><option value="all">جميع الأولويات</option>${priorities.map(x=>`<option ${state.agencyPriority===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label class="baseline-search">بحث<input id="agencySearchInput" value="${esc(state.agencySearch)}" placeholder="الكود، الطلب، المسؤول، المحفظة…"></label></div></section>${rows.length?`<section class="baseline-records">${rows.map((x,i)=>agencyRecord(x,i)).join('')}</section>`:'<div class="empty">لا توجد سجلات تطابق التصفية الحالية.</div>'}`;
}
