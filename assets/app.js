...
function agencyRows(){return DB.portfolios.flatMap(p=>workItemsFor(p,DETAILS[p.id]).map(item=>{const d=DETAILS[p.id]||{},date=agencyRequestDate(item,d),pillar=ARCH.pillars.find(x=>x.id===p.architecture.pillar)?.name||'غير مصنف';return{p,item,date,pillar,priority:agencyPriority(p),agencyOwner:(p.poc||p.boc)?.agency||'غير محدد',description:item.description||'غير مسجل',update:item.completionEvidence||item.requestEvidence||item.evidence||p.lastUpdate||'غير مسجل',center:item.centerRequirementId?`${item.centerRequirementId} — ${item.centerStatus||'حالة غير مسجلة'}`:'غير مرتبط بند محدد'}}))}
