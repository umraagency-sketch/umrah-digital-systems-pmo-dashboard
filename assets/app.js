...
function renderAgencyBaseline(){
  ...
  $('#servicesGrid').innerHTML=`<section class="center-baseline-overview"><article class="glass chart-card"><header><div><p class="eyebrow">مؤشر خط أساس الوكالة</p><h2>روستر المتطلبات وسجلات الأعمال</h2></div><span class="source-chip">قاعدة محافظ الوكالة · ${ar(all.length)} سجل</span></header><div class="agency-kpi-groups"><section><header><b>حالة التنفيذ</b><span>من إجمالي ${ar(all.length)} سجلاً</span></header><div class="agency-kpi-grid"><div class="total"><b>${ar(all.length)}</b><span>إجمالي السجل</span><small>${ar(completed)} مكتمل + ${ar(remaining)} متبقٍ</small></div><div class="completed"><b>${ar(completed)}</b><span>منفذة / مكتملة</span><small>وفق الحالة المسجلة</small></div><div class="remaining"><b>${ar(remaining)}</b><span>متبقية</span><small>قيد التنفيذ أو الإجراء أو الدراسة</small></div></div></section><section><header><b>حالة المطابقة مع المركز</b><span>من إجمالي ${ar(all.length)} سجلاً</span></header><div class="agency-kpi-grid"><div class="total"><b>${ar(all.length)}</b><span>إجمالي السجل</span><small>${ar(linked)} مطابق + ${ar(all.length-linked)} للمراجعة</small></div><div class="matched"><b>${ar(linked)}</b><span>مرتبطة بخط الأساس</span><small>برقم طلب محدد من المركز</small></div><div class="unmatched"><b>${ar(all.length-linked)}</b><span>تحتاج مراجعة أو مطابقة</span><small>لا يوجد ربط بندي مؤكد بعد</small></div></div></section></div></article></section><section class="baseline-source-head"><div><span>خط الأساس التشغيلي للوكالة</span><b>المتطلبات وسجلات الأعمال المسجلة في المحافظ</b><small>اسحب أي سجل وأفلته لتغيير ترتيبه العام. «منفذ/مكتمل» يعتمد على الحالة المسجلة فقط، وأولوية المحفظة تبقى تصنيفاً مستقلاً.</small></div><div class="baseline-filters"><label>المحفظة<select id="agencyPortfolioFilter"><option value="all">جميع المحافظ</option>${portfolios.map(x=>`<option ${state.agencyPortfolio===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label>الحالة<select id="agencyStatusFilter"><option value="all">جميع الحالات</option>${statuses.map(x=>`<option ${state.agencyStatus===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label>الأولوية<select id="agencyPriorityFilter"><option value="all">جميع الأولويات</option>${priorities.map(x=>`<option ${state.agencyPriority===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label class="baseline-search">بحث<input id="agencySearchInput" value="${esc(state.agencySearch)}" placeholder="الكود، الطلب، المسؤول، المحفظة…"></label></div></section>${rows.length?`<section class="baseline-records agency-sortable">${rows.map((x,i)=>agencyRecord(x,i)).join('')}</section>`:'<div class="empty">لا توجد سجلات تطابق التصفية الحالية.</div>'}`;
}
function agencyRecord(x,index){return`<details class="baseline-record agency-draggable" draggable="true" data-agency-key="${esc(x.key)}"><summary><span class="agency-drag-handle" title="اسحب لتغيير الأولوية">⋮⋮</span><span class="record-index">${ar(index+1)}</span><div class="record-identity"><b>${esc(x.item.id)}</b><small>${esc(x.item.title)}</small></div><span class="record-owner"><i>المسؤول في الوكالة</i>${esc(x.agencyOwner)}</span><span class="record-status"><i>الحالة</i>${esc(STATUS[x.item.status]||x.item.status)}</span><span class="record-priority"><i>أولوية المحفظة</i>${esc(x.priority)}</span><span class="record-response"><i>تاريخ الطلب/القيد</i>${esc(x.date.value)}</span><span class="record-chevron">⌄</span></summary><div class="record-details"><section class="record-group"><h3>التعريف والارتباط</h3><div><dl><dt>ترتيب الأولوية العام</dt><dd>${ar(index+1)}</dd></dl><dl><dt>المحفظة</dt><dd>${esc(x.p.name)}</dd></dl><dl><dt>المسار</dt><dd>${esc(x.pillar)}</dd></dl><dl><dt>المسؤول في الوكالة</dt><dd>${esc(x.agencyOwner)}</dd></dl><dl><dt>الربط مع المركز</dt><dd>${esc(x.center)}</dd></dl></div></section><section class="record-group"><h3>تفاصيل الطلب</h3><div><dl><dt>الطلب</dt><dd>${esc(x.item.title)}</dd></dl><dl><dt>التوضيح المسجل</dt><dd>${esc(x.description)}</dd></dl><dl><dt>تاريخ الطلب/القيد</dt><dd>${esc(x.date.value)} · ${esc(x.date.source)}</dd></dl><dl><dt>الأولوية</dt><dd>${esc(x.priority)} · موروثة من المحفظة</dd></dl></div></section><section class="record-group"><h3>المتابعة التنفيذية</h3><div><dl><dt>ما تم / آخر تحديث</dt><dd>${esc(x.update)}</dd></dl><dl><dt>الدليل</dt><dd>${esc(x.item.requestEvidence||x.item.evidence||'غير مسجل')}</dd></dl><dl><dt>الإجراء القادم</dt><dd>${esc(x.item.next||'غير مسجل')}</dd></dl><dl><dt>الدعم المطلوب</dt><dd>${esc(x.item.support||'غير مسجل')}</dd></dl></div></section></div></details>`}
function reorderAgency(from,to){if(from===to)return;const current=agencyRows().map(x=>x.key),fi=current.indexOf(from),ti=current.indexOf(to);if(fi<0||ti<0)return;const[item]=current.splice(fi,1);current.splice(ti,0,item);writeLocal(AGENCY_ORDER_KEY,current);renderAgencyBaseline()}
